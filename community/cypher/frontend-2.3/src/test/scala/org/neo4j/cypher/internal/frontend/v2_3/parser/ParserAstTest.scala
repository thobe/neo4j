...
trait ParserAstTest[AST] extends ParserTest[AST, AST] with TestName {
  ...
  final def param(name: String) = ast.Parameter(name)(_)

  final def int(value: Long) = ast.SignedDecimalIntegerLiteral(value.toString)(_)

  final def hex(value: Long) = ast.SignedHexIntegerLiteral("0x" + java.lang.Long.toHexString(value))(_)

  final def oct(value: Long) = ast.SignedOctalIntegerLiteral("0" + java.lang.Long.toOctalString(value))(_)

  final def add(lhs: Expression, rhs: Expression): Expression = { pos => ast.Add(lhs(pos), rhs(pos))(pos) }

  final def sub(lhs: Expression, rhs: Expression): Expression = { pos => ast.Subtract(lhs(pos), rhs(pos))(pos) }

  final def mul(lhs: Expression, rhs: Expression): Expression = { pos => ast.Multiply(lhs(pos), rhs(pos))(pos) }

  final def div(lhs: Expression, rhs: Expression): Expression = { pos => ast.Divide(lhs(pos), rhs(pos))(pos) }

  final def pow(lhs: Expression, rhs: Expression): Expression = { pos => ast.Pow(lhs(pos), rhs(pos))(pos) }

  final def mod(lhs: Expression, rhs: Expression): Expression = { pos => ast.Modulo(lhs(pos), rhs(pos))(pos) }

  final def pos(expr: Expression): Expression = { pos => ast.UnaryAdd(expr(pos))(pos) }

  final def neg(expr: Expression): Expression = { pos => ast.UnarySubtract(expr(pos))(pos) }
  ...
  final def bne(lhs: Expression, rhs: Expression): Expression = { pos => ast.InvalidNotEquals(lhs(pos), rhs(pos))(pos) }
