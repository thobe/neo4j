...
object CastSupport {
  ...
  private class ArrayConverterWriter(typ: Class[_], transformer: (AnyRef) => ArrayValue)
    ...
    override def writeDate(epochDay: Long): Unit =
      write(DateValue.epochDate(epochDay))

    override def writeLocalTime(nanoOfDay: Long): Unit =
      write(LocalTimeValue.localTime(nanoOfDay))

    override def writeTime(nanosOfDayUTC: Long, offsetSeconds: Int): Unit =
      write(TimeValue.time(nanosOfDayUTC, ZoneOffset.ofTotalSeconds(offsetSeconds)))

    override def writeLocalDateTime(epochSecond: Long, nano: Int): Unit =
      write(LocalDateTimeValue.localDateTime(epochSecond,nano))

    override def writeDateTime(epochSecondUTC: Long, nano: Int, offsetSeconds: Int): Unit =
      write(DateTimeValue.datetime(epochSecondUTC, nano, ZoneOffset.ofTotalSeconds(offsetSeconds)))

    override def writeDateTime(epochSecondUTC: Long, nano: Int, zoneId: String): Unit =
      write(DateTimeValue.datetime(epochSecondUTC, nano, ZoneId.of(zoneId)))
  }
